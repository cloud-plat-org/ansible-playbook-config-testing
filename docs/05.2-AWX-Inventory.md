...
## 📋 **WSL Lab Inventory Configuration**
...
declare -A HOSTS
HOSTS[ubuntuAWX]="172.22.192.129:2225"
HOSTS[argo_cd_mgt]="172.22.192.129:2226"
HOSTS[wslkali1]="172.22.192.129:2224"
HOSTS[wslubuntu1]="172.22.192.129:2223"
```
...
```bash
# Create the all_servers group in the inventory
awx --conf.host https://localhost -k --conf.token "$AWX_TOKEN" group create \
  --name "$GROUP_NAME" \
  --inventory "$INVENTORY_ID"
```

### **Step 2: Add All Hosts Using For Loop**
```bash
# Define host configurations
declare -A HOSTS
HOSTS[ubuntuAWX]="172.22.192.129:2225"
...
# Add all hosts to inventory
for host_name in "${!HOSTS[@]}"; do
  IFS=':' read -r ip port <<< "${HOSTS[$host_name]}"
  echo "Adding host: $host_name ($ip:$port)"
  
  awx --conf.host https://localhost -k --conf.token "$AWX_TOKEN" -f json --conf.color false hosts create \
    --name "$host_name" \
    --inventory "$INVENTORY_ID" \
    --variables "{\"ansible_host\": \"$ip\", \"ansible_port\": $port, \"ansible_user\": \"daniv\", \"ansible_ssh_private_key_file\": \"/home/daniv/.ssh/id_rsa\"}"
done
```

### **Step 3: Add All Hosts to Group**
```bash
# Add all hosts to the all_servers group
for host_name in "${!HOSTS[@]}"; do
...
```bash
# Check if all hosts were added successfully
...
# Check if all hosts are in the group
echo "=== Hosts in Group ==="
awx --conf.host https://localhost -k --conf.token "$AWX_TOKEN" -f json --conf.color false groups list_hosts --group "$GROUP_NAME" | jq '.results[] | .name'
...
```bash
# Remove specific host from group
awx --conf.host https://localhost -k --conf.token "$AWX_TOKEN" groups disassociate_host \
  --group "$GROUP_NAME" \
  --host "ubuntuAWX"
```
...
```bash
# Remove all hosts from the group
for host_name in "${!HOSTS[@]}"; do
...
### **Delete Hosts Entirely**
```bash
# Delete specific host (removes from ALL inventories)
awx --conf.host https://localhost -k --conf.token "$AWX_TOKEN" hosts delete "ubuntuAWX"

# Delete all hosts (removes from ALL inventories)
for host_name in "${!HOSTS[@]}"; do
...
<!--
Inventory "WSL Lab"
├── Host "ubuntuAWX" (has IP, port, user, SSH key, etc.)
├── Host "wslkali1" (has IP, port, user, SSH key, etc.)
└── Groups:
    ├── "all_servers" → references existing hosts by name
    └── "web_servers" → references existing hosts by name -->
